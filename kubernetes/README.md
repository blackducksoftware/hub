...
- Note: Postgres is known to have problems running in a container when writing to *Gluster-based* persistent volumes. (See [here](https://bugzilla.redhat.com/show_bug.cgi?id=1512691) for details.) If you are using Gluster for your underlying file system, then you shoud probably use an *external* database.
...
to your organization's use of the Hub (for example, external proxys might be needed).
